...
void Toep_3x3x3x2x2x8(uint16x8_t *toep, uint16x8_t *vec, uint16x8_t *r){
    ...
    for (uint16_t i=0; i<72; i++){
        ...
        toep_0_m1_m2 = vaddq_u16(toep_0 , vaddq_u16(toep_m1, toep_m2));
        toep_p1_0_m1 = vaddq_u16(toep_p1, vaddq_u16(toep_0 , toep_m1));
        toep_p2_p1_0 = vaddq_u16(toep_p2, vaddq_u16(toep_p1, toep_0 ));
    }
    ...
    // Recombination
    for (uint16_t i=0; i<36; i++){
        r[i     ] = vsubq_u16(intermediate_0[i], vaddq_u16(intermediate_3[i], intermediate_5[i])); 
        r[i + 36] = vsubq_u16(intermediate_1[i], vaddq_u16(intermediate_5[i], intermediate_4[i])); 
        r[i + 72] = vsubq_u16(intermediate_2[i], vaddq_u16(intermediate_4[i], intermediate_3[i])); 
    }
}
...
void Toep_3x3x2x2x8(uint16x8_t *toep, uint16x8_t *vec, uint16x8_t *r){
    ...
    for (uint16_t i=0; i<12; i++){
        r[i     ] = vsubq_u16(intermediate_0[i], vaddq_u16(intermediate_3[i], intermediate_5[i])); 
        r[i + 12] = vsubq_u16(intermediate_1[i], vaddq_u16(intermediate_5[i], intermediate_4[i])); 
        r[i + 24] = vsubq_u16(intermediate_2[i], vaddq_u16(intermediate_4[i], intermediate_3[i])); 
    }
}
...
void Toep_3x2x2x8(uint16x8_t *toep, uint16x8_t *vec, uint16x8_t *r){
    ...
    for (uint16_t i=0; i<4; i++){
        r[i    ] = vsubq_u16(intermediate_0[i], vaddq_u16(intermediate_3[i], intermediate_5[i])); 
        r[i + 4] = vsubq_u16(intermediate_1[i], vaddq_u16(intermediate_5[i], intermediate_4[i])); 
        r[i + 8] = vsubq_u16(intermediate_2[i], vaddq_u16(intermediate_4[i], intermediate_3[i])); 
    }
}
...
void Toep_2x2x8(uint16x8_t *toep, uint16x8_t *vec, uint16x8_t *r){
    ...
    uint16x8_t intermdiate_0[ 2 ];
    uint16x8_t intermdiate_1[ 2 ];
    uint16x8_t intermdiate_2[ 2 ];
...
void Toep_2x8(uint16x8_t *toep, uint16x8_t *vec, uint16x8_t *r){
    ...
    uint16x8_t intermdiate_0;
    uint16x8_t intermdiate_1;
    uint16x8_t intermdiate_2;
...
void TMVP_8x8_on_8(uint16x8_t *toep, uint16x8_t *vec, uint16x8_t *r){
    r = vdupq_n_u16(0);
    uint16_t scalar;
    uint16x8_t toep_vector[2] = {toep[0], toep[1]};

    scalar = vec[0];
    //toep[0] = vextq_u16(toep[0], toep[1], 1);
    r = vmlaq_n_u16(r, toep_vector, scalar);
    scalar = vec[1];
    toep[0] = vextq_u16(toep[0], toep[1], 1);
    r = vmlaq_n_u16(r, toep_vector, scalar);
    scalar = vec[2];
